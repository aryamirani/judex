...
export default function App() {
  ...
  const handleSwitchCam = async (targetCam) => {
    ...
    if (mode === 'live') {
      ...
      // In review mode, all cameras share the exact same local timeline generated by the unified DVR snapshot.
      // We simply copy the currentTime exactly.
  ...
  const handleSeek = (time) => {
    ...
    if (video) {
      if (mode === 'review') {
        ...
        // Seek all cameras simultaneously in review mode so they stay perfectly in sync
        CAMERAS.forEach(cam => {
          if (videoRefs[cam].current) {
            videoRefs[cam].current.currentTime = localTime
          }
        })
      } else {
